...
async function getGeminiResponse(currentMessage: string, history: Message[]): Promise<{text: string, sources: any[]}> {
    ...
    const apiKey = "AIzaSyCe_m6MJ8Jn2YI7RyqjVW-AnESX8lkThcc"; // The environment will provide the key.
    ...
    try {
        ...
        if (candidate?.content?.parts?.[0]) {
            ...
            // Ensure groundingMetadata and webSearchQueries exist before trying to map them.
            const sources = candidate.groundingMetadata?.webSearchQueries?.map(q => q.results).flat().filter(Boolean) || [];
            return { text, sources };
        } else {
        ...
        {message.sources && message.sources.length > 0 && (
            <div className="mt-3 pt-2 border-t border-gray-300">
                ...
                <ul className="list-none pl-0">
                    {/* --- FIX IS HERE --- */}
                    {/* We add .filter() to remove any invalid/empty sources before trying to render them. */}
                    {message.sources
                        .filter(source => source && source.uri) // This ensures the source object and its 'uri' property exist.
                        .map((source, index) => (
                 ...
                 <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-end gap-2 justify-start">
                    ...
                    <div className="max-w-[80%] rounded-xl px-4 py-3 text-sm shadow-md bg-gray-200 text-gray-800 rounded-bl-none">
                        <motion.div className="flex gap-1.5" initial="start" animate="end" transition={{ repeat: Infinity, staggerChildren: 0.2 }}>
                            <motion.span variants={{start: {y: "0%"}, end: {y: "100%"}}} transition={{duration: 0.4, yoyo: Infinity}} className="w-2 h-2 bg-gray-400 rounded-full" />
                            <motion.span variants={{start: {y: "0%"}, end: {y: "100%"}}} transition={{duration: 0.4, yoyo: Infinity, delay: 0.2}} className="w-2 h-2 bg-gray-400 rounded-full" />
                            <motion.span variants={{start: {y: "0%"}, end: {y: "100%"}}} transition={{duration: 0.4, yoyo: Infinity, delay: 0.4}} className="w-2 h-2 bg-gray-400 rounded-full" />
                        </motion.div>
                    </div>
                </motion.div>
            )}
